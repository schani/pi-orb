...
- **Forward-only script-repair fencing** (incident: `docs/postmortems/2026-08-06-rollover-repair-war-corrupt-image.md`). Two control-plane revisions with different expected startup scripts treat each other's script as stale and duel over `setMetadata`+start, hard-bouncing the VM. Candidate fix: stamp a monotonic script-generation number (or generation = deploy timestamp) in instance metadata and refuse to repair a host whose stamped generation is newer than the repairer's — an old revision then never repairs backward. `deploy.sh`'s drained-revision deletion is the operational mitigation, not the fix.
- **Detect a crash-looping runtime container as boot evidence** (same incident). A container that crashes before binding looks identical to "runtime never answered"; the boot-failure path should read the container's restart count/last exit (via `diagnose`) and surface it, and a crash-looping container whose image layers may be corrupt (hard stop mid-pull) should trigger image re-pull as recovery.
