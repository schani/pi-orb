...
The initial provider is hardcoded to Pi's built-in `openai-codex`, using a ChatGPT Plus/Pro subscription rather than an OpenAI API key. There are no model/provider/thinking-level environment variables or model-selection controls in the first slice. The runtime pins the first *image-capable* model in Pi's Codex catalog, falling back to the catalog head (`pickCodexModel`, decided 2026-08-01): the runtime advertises `input.image`, and pi-ai's serializers silently drop image content when `model.input` lacks `"image"` — discovered when a pasted screenshot reached the persisted session but never the model because the catalog head, `gpt-5.3-codex-spark`, is text-only. Default thinking level is unchanged; explicit model selection can be added only when the product needs it.
