...
Run `npm run test:e2e` before deploying any change that touches the runtime protocol, the runtime HTTP/WebSocket server, or the agent harness — the unit suite does not exercise the browser↔runtime handshake end to end (learned 2026-08-04: a partially-scoped snapshot gate rejected every first message with `stale_head`; only the E2E path covers that flow). Known blocker (2026-08-04, DESIGN.md open question 34): on macOS Docker Desktop the E2E cannot currently pass at all — the host-run control plane cannot reach container bridge IPs (verified empirically; readiness always times out). Until the harness is fixed (port publishing or a containerized control plane), state this limitation instead of skipping silently.
