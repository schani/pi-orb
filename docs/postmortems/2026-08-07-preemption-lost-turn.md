...
- **At incident time, the `system_event` query returned no evidence** — the preemption had to be proven from guest serial logs. **Field correction 2026-08-12:** the same project now returned the expected `compute.instances.preempted` System Event for orb `6ceb79c1` with the direct filter `resource.type="gce_instance"`, numeric `resource.labels.instance_id`, the incident time range, and `logName:"cloudaudit.googleapis.com%2Fsystem_event"`. The earlier result was therefore a query/retention-time finding, not evidence that the project sink excludes System Events; the corresponding TODO is closed. A `zoneOperations.list` check remains available to future real-time `diagnose()` labeling because Cloud Logging is forensic evidence, not a provider observation contract.
