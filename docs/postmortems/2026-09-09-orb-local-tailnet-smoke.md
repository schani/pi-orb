...
The already-applied release requires corrected lifecycle and workload-identity gates against the same deployed generation, with the global release lock held. Their outcome and owned-fixture cleanup are recorded when complete; no new application apply is needed for a harness-only correction.
