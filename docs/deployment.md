...
- **Ownership migrations 023–024:** migration 023 was deployed and data-verified on 2026-09-17; migration 024 is on `main`, qualified, and not deployed. Both use the all-or-none `PI_ORB_ORIGINAL_USER_ID`, `PI_ORB_ORIGINAL_IDENTITY_ISSUER`, and `PI_ORB_ORIGINAL_IDENTITY_SUBJECT` tuple from a trusted operator; the migration runner supplies it to each. Migration 023 needs no input only when projects and legacy personal instructions are initial-empty. Migration 024 needs no input only when `credential_pointers` is empty. With a complete tuple, each transaction inserts or verifies the exact UUID/issuer/subject mapping; missing required input, partial or invalid tuples, and identity conflicts fail closed. Migration 023 assigns non-null project owners, installs per-owner project-name uniqueness, and copies personal content/revision/timestamp. Its production cutover used independently verified owner UUID `53da7ad4-6c53-4223-868e-0641bb4bcdd9`: all three projects received that owner with other metadata unchanged, and the one personal-instructions row retained content hash, revision, and timestamp. The three bootstrap variables were removed after verification; `PI_ORB_USER_ID` remains for smoke selection. Migration 024 preserves every pointer field while assigning `user_id`, then replaces the original `provider` primary key with `(user_id, provider)`. Neither guesses email, projects, row order, or a first user; creates a placeholder/null owner; retargets identity; or adds a compatibility phase. A migration 024 release must preserve supported old control-plane/runtime writes or atomically reject incompatible writes with a typed, durable, user-visible outcome before mutation, under the runtime/migration compatibility policy below. No such production execution or stage 3 deployment is authorized.
...
owner tuple was supplied; it must not be guessed from row order or email. Retry requires explicit independently verified
`PI_ORB_USER_ID`, `PI_ORB_ORIGINAL_USER_ID`,
