...
- Offer only the image-capable `openai-codex` catalog entries Astra (`gpt-6-astra`), Sol (`gpt-6-sol`), Terra (`gpt-5.6-terra`), and Luna (`gpt-6-luna`), in that order. These short names are user-facing. Missing or non-image-capable entries are omitted, preserving the existing image-input promise. The prototype's Anthropic names are not available providers. **Catalog verification (2026-09-23):** Pi 0.87.1's installed catalog and a forced successful refresh both expose GPT-6 Luna with image input; neither exposes `gpt-6-terra`, so Terra remains on 5.6.
