...
Auxiliary model calls in a scripted scenario must be matched by their dedicated model session, not by text copied from the user's request. Learned from the first auto-naming E2E run: the naming prompt quotes the first message, so a `userMessage` rule for the agent turn also matched the Luna call, returned the agent's bash tool script to a no-tools naming request, and interfered with the expected turn. Production naming calls use a fresh `pi-orb-auto-name-<uuid>` session; mock-mode naming uses the fixed `pi-orb-mock-auto-name` session solely so the scenario can select its naming response deterministically.
