...
## First production rollout — 2026-09-07

The generation-fenced foundation/application state split was adopted and applied, then clean commit `dfa5dcc` first deployed the native GCE provider. The final corrected release is commit `db8cb5d`, image `pi-orb-image-v-db8cb5d-ebd0a574514547e9`, ID `6342246443724741372`: 1,369,020,352 compressed bytes with a 20 GiB boot disk and retained 50 GiB workspace per orb. Before transition, a mode-0600 snapshot retained exact metadata, history and message hashes for all 51 archived orbs across two projects. Pricing and release timings are recorded in `docs/deployment.md`.

Public validation reached a native runtime, confirmed Docker stayed off until requested, and passed Docker, model/tool, browser, terminal, Stop/Start and workspace-retention checks. It exposed three release blockers: transient Rust DNS failure was not retried; an ops instance retained a process-local stopping marker after another role completed Stop/Start; and a systemd ordering cycle dropped the workspace mount job on retained-disk image replacement. The first two fixes then passed the complete release smoke. Evidence and fixes are covered by `docs/postmortems/2026-09-07-native-rust-dns-bootstrap.md`, `docs/postmortems/2026-09-07-stale-stopping-marker.md`, and `docs/postmortems/2026-09-07-native-workspace-systemd-cycle.md`. The failed replacement VM was removed while its 50 GiB workspace stayed intact for the corrected incarnation-2 validation. The mount-corrected incarnation-2 replacement used image ID `6342246443724741372` and passed exact workspace, home, Docker-volume, history and model/tool persistence. An immediate terminal passed two seconds after `running`. The same incarnation then passed Stop/Start with every marker intact and Docker still off by default; the fixture was deleted. Post-deployment verification also confirmed all 51 archived orbs retained exact metadata, history and message hashes. The final release smoke passed lifecycle plus live issuer mint, verification, denial and revocation, and cleaned its fixtures. Its optional GCP STS federation leg was not configured; the retained repository fixture independently passed its scoped workload-identity helper.

The release used an administrator. It therefore does not close the separate requirement to prove the recurring deployer's scoped build, IAP SSH and application authorities through a complete release. The repository-scoped workload-identity helper passed in the deployed orb, but a release authenticated through the scoped orb WIF tier remains untested.

## Production fresh-boot timing — 2026-09-08

A production orb created at 00:37:53.392 UTC reached control-plane `running` at 00:44:51.865, 418.473 seconds later. VM provisioning and OS startup consumed 34.068 seconds before the workspace service's first structured edge. Reading and formatting the new 50 GiB disk consumed 337.608 seconds. The remaining 46.797 seconds covered mount, metadata bootstrap, runtime startup and the final control-plane poll.

After formatting, mount and bootstrap completed in 1.242 seconds. The runtime listened 13.463 seconds after its service started and declared ready 31.060 seconds after listening. That last interval includes Rust initialization, repository clone, setup, project-secret and model-auth checks, session creation, and resume; those operations lack individual durable timestamps, so this measurement does not assign time among them. Resume output began 1.683 seconds before readiness and included the expected gcloud external-account and registry configuration. The control plane observed readiness 1.015 seconds after the guest edge. A second new orb independently spent 337.188 seconds in the same workspace scan before proceeding normally. These findings motivate the preformatted workspace decision in `docs/host-provider.md`.

## Production preformatted-workspace release — 2026-09-08

Commit `17a29df` deployed runtime image `pi-orb-image-v-17a29df-917cd8030a9b45b5`, ID `2128921231866645132`, and workspace image `pi-orb-image-workspace-v-17a29df-917cd8030a9b45b5`, ID `991087715442412255`. They measured 1,369,415,936 and 67,328 compressed bytes respectively. The accepted 10 GiB ext4 template created a 50 GiB workspace; guest validation and growth took 589 ms.

The fresh orb reached durable `running` 82.562 seconds after API creation, or 83.549 seconds from the external request until observation. API creation to workspace preparation took 41.308 seconds, workspace-ready to runtime listener took 16.342 seconds, listener to runtime-ready took 21.253 seconds, and runtime-ready to the durable transition took 3.071 seconds. Setup took 37 ms and resume 6.333 seconds. The other 14.883 seconds between listener and ready include Rust, clone and remaining runtime initialization, without exclusive per-step attribution. These measurements do not retroactively apportion the historical 31.060-second interval above.

The release checks passed 1,250 tests with one skip, 30 guest tests, real-ext4 acceptance and the complete E2E suite. Lifecycle smoke measured 70 seconds for fresh creation and 56 seconds for restart; issuer mint, verification, denial and revocation also passed. The fresh orb accepted an immediate terminal, kept Docker off, passed an actual GCP read through the repository workload-identity helper, rejected the wrong audience, and was deleted with its owned VM and disk. The optional built-in federation environment legs were not configured. Archive verification found the original 51 archived orbs across two projects unchanged.
