...
**Deployment ordering (2026-09-16; invariant reinforced 2026-09-17).** Before applying a one-time shape backfill, prove every incompatible runtime writer is stopped; keep them fenced through predicate verification, then restart them on the new image. The `022_typed_history_fields.sql` rollout omitted that fence while one legacy runtime kept writing. An initial bounded check found no affected rows; a later cutoff found five missing patch projections after five legacy edit-tool writes. Their native patch data remains present, so replay can repair them with no observed native-data loss. Incident and recovery: `docs/postmortems/2026-09-17-typed-history-runtime-fence.md`; release contract: `docs/deployment.md`.
