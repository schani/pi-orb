...
- The runtime always appends a concise tool-baseline section to Pi's system prompt: Python 3 and Rust are available; Rust uses a persistent rustup installation with stable as the default and supports repository `rust-toolchain.toml` selection. This composes after discovered `APPEND_SYSTEM.md` content and before the optional port-exposure section without replacing Pi's normal resource discovery (decided 2026-08-09; tool details in `docs/host-provider.md`).
