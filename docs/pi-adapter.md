...
- Pinned by `apps/orb-runtime/src/pi/resource-loader.contract.test.ts` (the skill is discovered through `getSkills()` against the pinned SDK, with the loader otherwise byte-identical to the control loader, and an absent directory yields no diagnostics), `apps/orb-runtime/src/pi/skills.test.ts` (frontmatter parses; every `/usr/local/bin` path the skill names is installed by the image), and `apps/orb-runtime/src/dockerfile.contract.test.ts` (both COPY lines).
