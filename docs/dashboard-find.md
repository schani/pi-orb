...
3. **Route adapter** — the active route supplies an `AppSearchSource`. The dashboard adapter maps already-loaded `ProjectView` and `OrbView` state into generic items and status. `OrbIndex` reuses that adapter with its own project/orb snapshots and age clock. Its source ID includes the current orb ID so changing orbs closes Find and clears the query even though the index stays mounted. Other surfaces can provide different sources without modifying the shell or core.
