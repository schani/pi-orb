...
- **Open-tab turn notifications (decided 2026-08-06).** The orb page offers an explicit browser-notification permission action. When a connected page receives a live-only Luna `turn_notification`, it displays an OS notification only if permission is granted and the page is hidden or unfocused. The orb/operation pair is the notification tag and local deduplication key; clicking focuses and navigates to the orb. Same-tag browser replacement suppresses duplicates from multiple open tabs. Notifications are deliberately best-effort: no service worker, Web Push subscription, persistence, or reconnect replay exists, so closing all orb pages means receiving no notification.
