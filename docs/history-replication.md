...
Drain failures are classified. Transport failures, `503 history_unavailable`, and transient database errors are retryable. `409 cursor_not_found`, a session-header mismatch, and mapping/validation failures are replication-integrity failures that no retry can repair. On an integrity failure the control plane abandons the drain and one transaction marks the orb `failed` with a typed error, revokes the runtime token, and records a discard fence through the current compute incarnation. Shared replacement-intent reconciliation then disposes compute while preserving the authoritative filesystem, which still holds every complete record for a future reconciliation mechanism (see open questions) to repair the replica.

That order — claim the row, then dispose compute — is required, not incidental (DST-found 2026-08-06; generalized 2026-08-12). Stopping first raced the drain reconciler of the same `stopping` orb: it observed the host the integrity path just stopped, took the "already-stopped host cannot be drained" branch, and CASed the orb to a clean `stopped`, discarding the integrity signal. The durable failure/discard transaction now wins first, and the shared incarnation-bounded disposer is the sole cleanup owner (`docs/compute-replacement.md`).
