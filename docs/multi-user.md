...
**Decision (2026-09-16):** stage 1 application identity is deployed for existing single-user use from `1fcc261`. Stage 2 owned projects and personal settings is implemented and qualified on `main`, but not deployed. Stage 3 credentials is authorized for implementation. Stages 1–3 must be complete before coworker onboarding.
...
- Stage 2 gives projects required owners, per-owner names and default lists, and user-keyed personal instructions. It is on later `main`, not in deployed source `1fcc261`.
