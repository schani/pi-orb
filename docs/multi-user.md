...
**Decision (2026-09-16):** stage 1 application identity is on `main` and stage 2 owned projects and personal settings is implemented and qualified. Neither is deployed. Stage 3 credentials is authorized for implementation; deployment remains unauthorized. Stages 1–3 must be complete before coworker onboarding. Earlier stages can be deployed for existing single-user use after normal release qualification.
...
- The local implementation resolves an application principal and makes `/api/v1/session` a no-store principal probe. The deployed revision still has the earlier reachability-only response until an authorized identity deployment. See `docs/deployment.md` and `docs/control-plane-api.md`.
- Stage 2 gives projects required owners, per-owner names and default lists, and user-keyed personal instructions. The deployed service is unchanged.
...
## Application identity (on `main` at `0746680`; not deployed)
...
The original user is never inferred from email, row order or a browser bootstrap call. Migration 023 receives an explicit trusted-operator identity tuple as described below and creates or verifies that stable mapping transactionally. Stage 1 can be tested locally without deployment.
...
Use deterministic schedules for multi-user login/refresh races and existing lifecycle integrations, store contracts for schema/query changes, and browser/runtime E2E for two logged-in users. This assessment involved repository inspection and Tailscale documentation, not deployment or live security qualification. Question 24 records the selected stage-2 access scope; remaining stage-3 and tailnet choices live in questions 25–26. Stage 1 is on `main`; stage 2 is implemented and qualified. Qualification is recorded in `docs/testing.md`; neither has changed the deployed service.
