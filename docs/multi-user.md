...
**Decision (2026-09-16; deployment authorization updated 2026-09-17):** stage 1 application identity is deployed for existing single-user use from `1fcc261`. Stage 2 owned projects and personal settings is implemented, qualified and authorized for production deployment, pending explicit independently verified original-owner inputs. Stage 3 credentials is authorized for implementation but is not part of this release. Stages 1–3 must be complete before coworker onboarding.
...
- The deployed stage-1 revision resolves an application principal and makes `/api/v1/session` a no-store principal probe. Cloud request logs observed guarded API traffic, but no dedicated session-response or two-user test was run. See `docs/deployment.md` and `docs/control-plane-api.md`.
- Stage 2 gives projects required owners, per-owner names and default lists, and user-keyed personal instructions. It is authorized for deployment from later `main`, but production remains at `1fcc261` pending explicit independently verified original-owner inputs.
...
**Clarification (2026-09-16; updated 2026-09-17):** the earlier blanket ban on deploying stage 1 independently conflated deployment with coworker onboarding. Identity alone leaves shared data and credentials, so it is not a complete multi-user release. There is no inherent dependency preventing a qualified single-user deployment of an earlier stage. All three stages must be complete before onboarding coworkers: partial delivery must not serve another user the original user's global credentials or instructions. No dual-read, legacy fallback or compatibility rollout is proposed. Stage 1 is deployed for single-user use; stage 2 is implemented, qualified and authorized for deployment once explicit verified owner inputs are supplied. Stage 3 is authorized for implementation but is not part of the stage-2 release.
...
2. **Owned projects and personal settings (implemented, qualified and authorized; pending verified owner inputs).** Migration 023 adds required project ownership, per-owner project-name uniqueness and user-keyed `personal_instructions`. Default browser lists, creation and personal settings use the signed-in user; the four corresponding ops operations require explicit known-user selection. Runtime list inspection defaults to the calling orb owner's projects, runtime boot uses the project owner, and explicit transcript reads plus existing direct resource access remain company-wide. Tests cover migration fail-closed/rollback behavior, duplicate names across users, owner-aware idempotency, inheritance, next-start instruction adoption and viewer-independent owner selection. Exit: two test users have independent default lists and instructions without duplicating ownership on child tables.
...
Use deterministic schedules for multi-user login/refresh races and existing lifecycle integrations, store contracts for schema/query changes, and browser/runtime E2E for two logged-in users. This assessment involved repository inspection and Tailscale documentation, not deployment or live security qualification. Question 24 records the selected stage-2 access scope; remaining stage-3 and tailnet choices live in questions 25–26. Stage 1 is deployed from `1fcc261`; stage 2 is implemented and qualified on later `main`, authorized for deployment, and pending explicit verified owner inputs. Qualification and deployment evidence are recorded in `docs/testing.md` and `docs/deployment.md`.
