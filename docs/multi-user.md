...
- Stage 3 on `main` gives each owner independent Codex/GitHub pointers, login gates, challenges, and Pi login artifacts. It remains undeployed and unauthorized for coworker onboarding.
...
Issuer and subject stay server-only. Identity resolution is separate from the ownership and credential scopes below.
...
Migration `024` uses the same all-or-none `PI_ORB_ORIGINAL_USER_ID`, `PI_ORB_ORIGINAL_IDENTITY_ISSUER`, and `PI_ORB_ORIGINAL_IDENTITY_SUBJECT` tuple as migration `023`. An empty `credential_pointers` table needs no tuple. Existing pointers require it. The migration inserts or verifies the exact UUID/issuer/subject mapping, fails on missing, partial, invalid, or conflicting input, assigns every existing pointer to that user, makes `user_id` non-null, and replaces the original `provider` primary key with `(user_id, provider)` in one transaction. It can seed the explicit original user even when migration `023` was already applied to empty owned data. It preserves generations, row versions, leases, exact secret versions, refresh times, and timestamps. It never guesses from projects, row order, email, or a first user. The migration runner supplies the tuple to both ownership migrations. A production release must preserve supported old control-plane/runtime writes or atomically reject incompatible writes with a typed, durable, user-visible outcome before mutation, as required by `docs/deployment.md`. Deployment remains unauthorized.
...
3. **Per-user Codex/GitHub end to end (on `main`, qualified, undeployed).** Migration 024 and user-bound brokers key pointers, refresh leases, login state, Pi artifacts, lifecycle cohorts, runtime grants, naming, and auxiliary inference by project owner. Shared provider clients and secret parents remain generic. Owner-only views receive actionable device codes; coworkers and ops receive only `owner_login_required` plus provider. DST, SQL contracts, route/browser coverage, and process E2E prove one user's credential failure neither overwrites nor blocks the other's healthy flow, and model/Git activity uses the orb owner's identity. Qualification evidence is in `docs/testing.md`; deployment and coworker onboarding are not authorized.
