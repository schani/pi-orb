...
**Decision (2026-09-16; deployment updated 2026-09-17):** stages 1–2 application identity, owned projects, and personal settings are deployed from `ec81e80` for existing single-user use. Migration 023's owner assignment and data preservation are verified. The release's typed-history runtime fence remains incomplete, so this is not a fully qualified rollout (`docs/postmortems/2026-09-17-typed-history-runtime-fence.md`). Stage 3 credentials is authorized but not deployed. It must complete before coworker onboarding.
