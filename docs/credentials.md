...
**Cloud exposure.** Four Cloud Run services from the same image, gated by a role env var that controls which routes are *registered* (a hard allowlist, not hidden-by-convention): the browser-facing service (behind IAP, min one instance, instance-based billing, runs the poller and the login flow) a tooling-facing `ops` service (the browser API surface with no loops, migrations, or web assets; public ingress but invoker-IAM restricted to the `pi-orb-debug` service account, reached via impersonated ID tokens — `infra/api.sh`), and a runtime-facing service registering only `/runtime/v1/*`, with `ingress=internal`, unauthenticated at the Cloud Run IAM layer by explicit decision — caller authentication is the orb token; requiring Google identity tokens from the VM would push GCP-specific auth into the runtime, which stays provider-agnostic (URL + bearer token on every provider). Both properties (internal ingress, route allowlist) are enforced in OpenTofu. The previously sketched fallback — one service behind an external load balancer with `ingress=internal-and-cloud-load-balancing` — was rejected: VPC-internal callers (i.e., orbs) could reach the `run.app` origin directly and bypass IAP entirely, so it would be safe only with app-level verification of signed IAP assertions on every browser route. The fourth service, `pi-orb-issuer`, was added to the OpenTofu root on 2026-08-21 (`docs/workload-identity.md` stage 4; live validation pending): it serves only the public OIDC discovery document and JWKS — no orb-data route, no secret-store access, no authentication — and is the deployment's first and only deliberately public unauthenticated surface. It is also the one service that does *not* run as the shared `pi-orb-control-plane` account: it has its own `pi-orb-issuer` service account whose only secret access is the database URL, so the unauthenticated surface and the identity that can read every brokered credential and the private signing keys are different principals. That separation is a Secret Manager boundary and stops there (POC limitation, recorded 2026-08-22): the database URL it reads is the deployment's single full read/write application credential, so at the PostgreSQL layer the public service holds the same rights as the others — it could read `orbs.runtime_token_hash` or write `oidc_signing_keys` if its code asked, and only the role's route allowlist and trimmed environment stop it. A read-only PostgreSQL role for the issuer, granted `SELECT` on `oidc_signing_keys` alone, is tracked in `TODO.md`. The role env var is now an allowlist (`all | browser | runtime | ops | issuer`) that refuses to boot on anything else, and the `runtime` and `issuer` roles additionally require a valid `PI_ORB_OIDC_ISSUER_URL`, which OpenTofu computes for both rather than an operator supplying it (`docs/deployment.md`).
