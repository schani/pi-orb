...
> A compute incarnation is immutable. If the desired native image, host configuration, or host-specification fingerprint changes, the old incarnation is disposed and a new one is created around the retained workspace; it is never repaired in place.

“Compute incarnation” means disposable process/container/VM state. It does not mean the authoritative workspace:

- GCE: delete the instance and its `autoDelete: true` native boot disk; retain `pi-orb-data-<orbId>` (`autoDelete: false`).
...
**Native Debian Docker durability extension (decided 2026-09-05):** non-regenerable project Docker state belongs to the retained orb filesystem, including volumes; writable container layers and local-only images are not assumed regenerable. Platform boot/runtime remains disposable. Replacement recreates platform compute without erasing retained workload stores. Failures in those stores require data-preserving recovery and visible diagnosis.
...
- `host_spec_fingerprint` is the committed fingerprint of the immutable launch specification used for the current incarnation: exact native image resource and numeric image ID, composed non-secret runtime configuration, and machine/disk/network settings whose change requires replacement.
...
- the provider exposes a pure `desiredSpecFingerprint({ orbId, repositoryUrl })` calculation. It hashes the effective image, runtime configuration, machine, disk, network, service-account, and scheduling inputs through `apps/control-plane/src/adapters/spec-fingerprint.ts` (recursively key-sorted JSON, SHA-256). A deployment whose effective specification is unchanged produces the same fingerprint; any effective update produces a different one.
...
**The native image identity is GCE-composition-enforced (decided 2026-09-05).** `PI_ORB_GCE_IMAGE_RESOURCE` must name an exact `projects/.../global/images/...` resource, never a family, and `PI_ORB_GCE_IMAGE_ID` must carry its numeric Compute identity. Before any disk or instance mutation, the provider reads the image and rejects an ID mismatch. Both values participate in the fingerprint, preventing name reuse or a moving alias from changing host contents invisibly.
...
6. Replacement request advances `state_version`, invalidating every earlier lifecycle snapshot. New provision uses the advanced incarnation and current desired fingerprint, then commits the new host ref/token hash/fingerprint/generation only if state version and incarnation still match and no discard is pending. A losing provision pass discards its side effect only when the durable incarnation is higher. The orphan sweep removes any lower incarnation that survives a process crash; it never discards the durable current incarnation or the retained workspace.
...
Disposal deliberately trades away post-hoc host forensics. The 2026-08-06 root cause was proven by inspecting the still-existing VM’s filesystem; after this plan that VM is gone. The compensation is capturing evidence before deletion: the terminal decision folds the provider's `diagnose` output into the typed `last_error`, and before deleting GCE compute the routine calls the same best-effort path when the terminal failure did not already capture host evidence. GCE diagnosis reads the bounded structured boot-status guest attribute; the result is persisted in `host_discard_evidence` before provider deletion and included in the one discard outcome edge. Diagnosis failure is persisted as unavailable evidence but must not block disposal. Native runtime stdout/stderr remains only in the VM's journald and disappears with compute; native code must publish the bounded structured events needed after deletion to the `pi-orb-boot` log explicitly. The boot-status guest attribute and explicit Cloud Logging events retain the product/operator explanation after instance deletion. The 2026-08-12 incident was root-caused entirely from durable channels without touching the host, which is the bar this section maintains.
...
5. **Immutable specification**
   - Add the pure orb-specific `desiredSpecFingerprint` calculation with exact native-image resource and numeric-ID assertions; commit fingerprint plus deploy generation on provision.
   - Add the forward-only-fenced `host_spec_changed` intent in the start path with the declined edge.
   - Remove GCE `ensureCurrentScript`, `setMetadata` repair, and all `repaired` outcomes; a mismatch is a typed replacement requirement.
...
The test-only runtime launch seam is enabled solely by the E2E composition environment, not a production route or API. A workspace marker selects one exact orb/incarnation, exposes a typed terminal runtime failure, and durably records the injected event; the marker remains armed for that incarnation so process/container restart cannot heal suspect compute in place, while later incarnations launch normally. Tests wait on API state, durable store fields, provider inventory, and lifecycle edges. Negative lifecycle assertions drive three explicit reconciliation passes and await a request-correlated completion checkpoint for each pass. The checkpoint carries the scheduler generation, so an older in-flight pass cannot satisfy or overwrite the requested pass. This replaces the former 65-second failed-orb and 5-second running-spec observation windows without weakening their assertions. Every bounded wait dumps the orb row, provider inventory, and control-plane log tail on first timeout. E2E global setup rebuilds `pi-orb-runtime:dev` once before the serial lifecycle project, so files share one fresh image without racing to overwrite its tag.
...
4. Complete three explicitly requested terminal reconciliation passes with no replacement-provisioned event or compute, proving failure does not autonomously reprovision.
