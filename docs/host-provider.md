...
A minimal Debian image runs the orb runtime directly on a dedicated VM as a systemd service, with Docker Engine, Buildx, and Compose installed for project workloads. The VM is the isolation boundary. The existing one-VM/one-data-disk model and immutable replacement machinery in `docs/compute-replacement.md` remain authoritative. Docker, its socket, and containerd are disabled at boot; the agent starts Docker explicitly. The guest initializes only an entirely zeroed device without filesystem signatures and mounts `/workspace` before any dependent service. Nonblank, damaged, or unsupported storage fails closed.

**Faster new workspaces (decided 2026-09-08; implementation pending).** The full-device safety check above read a new 50 GiB disk for about 338 seconds of a measured 418-second fresh-orb boot. Build and accept a 10 GiB empty ext4 workspace image beside the native runtime image, and pin both its resource name and numeric identity in the release manifest and application configuration. Only creation of a missing data disk uses that image; retry and concurrent-create recovery verify the observed source identity before adopting the disk. The guest validates ext4, grows it to the provisioned disk size, and mounts it without formatting. Retained owned disks attach unchanged after template changes and require no source-image match. A missing or unsupported filesystem fails closed. Acceptance must cover ambiguous create responses, concurrent creation, retained-disk replacement and Stop/Start, image contents, and total fresh-boot time. A stale format flag was rejected because it can authorize destruction after disk reuse; binding one-shot authority to an exact disk would require a durable create/format/retry state machine without improving the image-backed result. Implementation and release evidence are tracked in `TODO.md`.
...
Native GCE boot timing (decided 2026-09-05): the guest verifies every byte before
formatting a new workspace disk. Real 50 GiB admission exceeded the old
...
runtime, within the fifteen-minute create/start deadline. Existing ext4 disks
skip the blank-device scan. Unit/DST tests cover slow success, bounded failure,
and unchanged Docker/process timing.
