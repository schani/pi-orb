...
**Separate runtime supervisor retained (decided 2026-09-08; implementation language open).** `infra/native-vm/runtime_supervisor.py` remains the current systemd entry point. systemd owns restart policy; the supervisor launches the runtime as a child, publishes ready, typed terminal health failures, and failures before runtime module load for each attempt, and forwards termination to the runtime process group. A direct runtime `main.ts` `ExecStart` remains rejected because it cannot report its own loader crash. Implementation-language and testing proposal: `docs/open-questions.md`, question 57.
