...
**Runtime observer retained (decided 2026-09-08).** `infra/native-vm/runtime_supervisor.py` remains the systemd entry point. systemd owns restart policy; the observer has no independent readiness deadline. For each attempt it publishes ready, typed terminal health failures, and failures before module load to guest attributes and Cloud Logging, and forwards termination to the runtime process group. A direct Node `ExecStart` was rejected because it loses precise early-exit evidence without replacement systemd hooks and per-attempt state.
