...
## AWS Lambda MicroVMs as a host provider (not adopted 2026-09-05)

Decision: retain GCE. Unverified storage integration, lifecycle changes, and cross-cloud operations outweigh the unmeasured startup benefit. The evaluated proposal follows for future reference.

The eight-hour lifetime is compatible with short compute sessions over a durable workspace. Proposed first path: drain and terminate on Stop, then create a fresh incarnation on Start. Exceptionally long sessions still need a deadline guard; seamless rotation and suspend/resume are optional extensions. This fits the immutable-compute direction in `docs/compute-replacement.md`, but stopped-without-compute and deadline handling need lifecycle work.

The storage constraint is decisive: the documented API has no attachable persistent block disk. Per-orb EFS through a VPC connector is plausible from the documented mount capabilities, but remains unverified. Mount all authoritative `/workspace` state, including home and session data; require the prior writer to be fenced before a successor starts. Object-store export on shutdown is rejected because abrupt failure can lose edits.

Snapshot restore offers fast compute startup; workspace mounting, credentials, and per-incarnation setup remain on the orb-ready path. AWS also requires ARM64/AL2023 image adaptation and cross-cloud networking/authentication. Current `RunMicrovm` documents idempotent creation through `clientToken`.

Full evaluation and sources: `docs/AWS-MICROVMS.md`. Decision and evaluation criteria: `docs/open-questions.md`, question 37.

## GCP sandbox alternatives (not adopted 2026-09-05)

GKE Agent Sandbox supports retained workspace block disks through existing PVCs. Normal attachment takes multiple seconds; the documented subsecond warm-pool example uses shared Filestore with custom privileged mount machinery. Cloud Run instances offer explicit per-instance lifecycle and documented NFS mounts, with no documented Persistent Disk attachment. Cloud Run sandboxes can bind-mount parent directories, but their near-instant creation assumes an existing parent instance and does not make its storage durable.

Decision: retain GCE; disk-attachment latency, NFS tradeoffs, and additional operations do not justify switching. If revisited, prioritize GKE for block storage or compare Cloud Run instances with AWS MicroVMs for NFS. Full evaluation and sources: `docs/GCP-SANDBOXES.md`; decision and evaluation criteria: `docs/open-questions.md`, question 47.
