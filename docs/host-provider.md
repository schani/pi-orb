...
- **Field finding 2026-08-11:** runtime image replacement is currently one-shot and destructive. The startup script removes the existing container before `docker run`; when the exact digest is absent, `docker run` implicitly pulls it once. A transient Artifact Registry header timeout therefore left the host with no runtime until another boot, causing the release smoke's real `runtime_never_answered` failure. The same digest pulled and started successfully on that later boot, ruling out a bad image. The accepted corrective direction is to pull the digest with bounded retry/backoff before removing the existing container and then run without pulling; tracked in `TODO.md` and `docs/postmortems/2026-08-11-release-smoke-restart-registry-timeout.md`.
