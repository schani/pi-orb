...
The unsandboxed process provider cannot supply image packages: it inherits host executables. `rustup` and Chromium are therefore documented local-test prerequisites; Linux also needs Python 3 and `build-essential` to compile `node-pty`, while macOS uses its bundled prebuild. The workspace dependency supplies the `agent-browser` CLI, and each orb still keeps home-relative Rust and browser state in its private persistent home.
...
- **Immutable-host replacement supersedes in-place repair (decided 2026-08-12; implementation planned in `docs/compute-replacement.md`).** The two preceding bullets remain the description of current deployed behavior and incident history, not the target contract. Remove `ensureCurrentScript`, metadata/script rewrites, script-generation stamps, and `repaired` outcomes. A runtime-image, startup-script, boot-image, machine, network, or other effective host-specification change produces a new fingerprint. Running orbs are not bounced; on the next ordinary Start, stale stopped compute is disposed and a higher-incarnation VM is created around the retained `pi-orb-data-<orbId>` disk. Same-fingerprint stop/start may reuse its VM. Failed compute is disposed immediately and replaced only after later user/wake intent. Incarnation-specific names prevent a delayed delete from reaching a replacement, and the replacement decision itself is fenced forward-only on the committed deploy generation — the surviving fence from the bullet above — so a draining stale revision cannot replace newer-spec compute backward.
- Host-side evidence channels (implemented 2026-08-06, from `docs/postmortems/2026-08-06-rollover-repair-war-corrupt-image.md`, where a container crash-looped for hours with nothing to look at). Three orthogonal channels, all set up by the GCE provider:
  ...
  Both metadata keys are *rewritten* by the startup-script repair path (as is `pi-orb-script-generation`), not merely preserved like the runtime token and the tailscale key, so hosts created before they existed adopt them on the repair that is their only upgrade path. `diagnose` queries both guest attributes and joins them: `startup-script: container-started imagePullAttempts=1; container: status=restarting restartCount=47 lastExitCode=1 at=…`. A failure reading the container attribute yields no container evidence rather than an Err — supplementary evidence must never make the whole diagnosis uncertain, because an Err defers the caller's boot-failure decision another poll.
...
- `provision` creates or reuses that directory by orb ID, mints a token only for a genuinely new host, chooses an unused loopback port, and launches `node apps/orb-runtime/src/main.ts` as a child with `PI_ORB_WORK_DIR=<host-dir>/workspace`, `HOME=<host-dir>/workspace/home`, the normal orb/bootstrap/broker environment, and a new `PI_ORB_RUNTIME_PORT=<assigned-port>`. The provider creates the private home before launch rather than inheriting the control-plane user's home. The runtime reads the optional port variable and defaults to 8080 for Docker/GCE.
