...
Three point-of-use CLI shims are installed into `/usr/local/bin` and are part of the baseline: `gh` and `pi-orb-git-credential` for broker-backed GitHub auth (`docs/credentials.md`), and `pi-orb` for workload identity (`docs/workload-identity.md`, added 2026-08-21). `pi-orb id-token --audience <audience> [--ttl-seconds <60..3600>]` mints a short-lived OIDC token from the control plane using the provider-injected runtime environment and prints the JWT plus one trailing newline to stdout, so command substitution and executable credential sources can consume it directly; failures are a concise stderr line and a per-class exit code. Each shim is a POSIX `sh` wrapper that execs a Node entry point in the image (`apps/orb-runtime/src/id-token/cli.ts` for `pi-orb`), and the Dockerfile contract test asserts all three are copied and made executable. Alongside them the image bakes `scripts/pi-orb-gcp-identity` at `/usr/local/bin/pi-orb-gcp-identity`, the reviewed executable credential source a Google external-account configuration names (`docs/workload-identity-recipes.md`, added 2026-08-22).

The image also bakes pi-orb's own Pi skills at `/opt/pi-orb/skills` — outside `/workspace`, whose persistent volume would shadow them — and the resource loader adds that path to Pi's skill discovery (`docs/pi-adapter.md`, added 2026-08-22). The first is `cloud-identity`, which teaches the in-orb agent how to federate with `pi-orb id-token`.

The unsandboxed process provider cannot supply image packages: it inherits host executables. `rustup` and Chromium are therefore documented local-test prerequisites; Linux also needs Python 3 and `build-essential` to compile `node-pty`, while macOS uses its bundled prebuild. The workspace dependency supplies the `agent-browser` CLI, and each orb still keeps home-relative Rust and browser state in its private persistent home. The shims above are likewise image-only there: the identity CLI remains invokable as its Node entry point, which accepts the `id-token` subcommand word the shim otherwise consumes, and it reads the same injected environment the image shim would. The baked skills directory is image-only for the same reason — there is no image, so `/opt/pi-orb/skills` does not exist and the loader omits it rather than reporting a missing resource.
