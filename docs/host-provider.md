...
- Host-side evidence channels (implemented 2026-08-06, from `docs/postmortems/2026-08-06-rollover-repair-war-corrupt-image.md`, where a container crash-looped for hours with nothing to look at). Three orthogonal channels, all set up by the GCE provider:
  ...
  Both metadata keys are *rewritten* by the startup-script repair path (as is `pi-orb-script-generation`), not merely preserved like the runtime token and the tailscale key, so hosts created before they existed adopt them on the repair that is their only upgrade path. `diagnose` queries both guest attributes and joins them: `startup-script: container-started; container: status=restarting restartCount=47 lastExitCode=1 at=…`. A failure reading the container attribute yields no container evidence rather than an Err — supplementary evidence must never make the whole diagnosis uncertain, because an Err defers the caller's boot-failure decision another poll.
...
- `provision` creates or reuses that directory by orb ID, mints a token only for a genuinely new host, chooses an unused loopback port, and launches `node apps/orb-runtime/src/main.ts` as a child with `PI_ORB_WORK_DIR=<host-dir>/workspace`, the normal orb/bootstrap/broker environment, and a new `PI_ORB_RUNTIME_PORT=<assigned-port>`. The runtime reads that optional variable and defaults to 8080 for Docker/GCE.
...
It does not reproduce the runtime image's package boundary, container networking, cgroups, Docker restart behavior, or image build, so Docker/GCE E2E and deploy smoke tests remain authoritative for those properties. PostgreSQL is not installed in the evaluated environment (no `postgres`, `initdb`, `pg_ctl`, or `psql`, and no Debian package indexes are configured). Rather than installing and supervising it, the recommended process-mode composition uses the test-only SQLite adapter proposed in `docs/stack.md`; database selection and location remain application configuration, not host-provider responsibilities. The root `npm run dev:local` script selects both adapters, making the whole management tree one command while the control plane remains the only long-lived parent service. Adapter tests cover configurable-path ownership, token-preserving stop/start, crash restart, SQLite CAS/transaction/history invariants, and credential-pointer CAS; the full-slice E2E passes with `PI_ORB_E2E_BACKEND=process`, including device login, real Pi tool and shell execution, live WebSocket handoff, replication, drain, and stopped history.
