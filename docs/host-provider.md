...
The unsandboxed process provider cannot supply image packages: it inherits host executables. `rustup` and Chromium are therefore documented local-test prerequisites; the workspace dependency supplies the `agent-browser` CLI, while each orb still keeps home-relative Rust and browser state in its private persistent home.
