...
gcloud run services describe pi-orb --project "$PROJECT" --region "$REGION" \
...
' "$service_status"); then
  echo "deploy failed: browser service does not route the files tag to 100% of its latest ready revision" >&2
  exit 1
fi
