...
The original native WebKit compositor SIGSEGV remains unresolved; later passing browser runs do not clear it. An initial read-only cloud preflight could not refresh the configured federated identity because the issuer did not answer within the CLI's ten-second budget (exit 6). A later independent preflight succeeded with the existing federation; no fallback admin login was attempted. Native/cloud and everyday-use qualification are not yet claimed. Outstanding gates are tracked only in `TODO.md`.
