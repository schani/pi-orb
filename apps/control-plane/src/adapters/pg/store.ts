...
export class PostgreSQLControlPlaneStore implements ControlPlaneStore {
  ...
  > {
    ...
    >(async (query) => {
      ...
      const autoStart = state === "stopping";
      const inserted = await query(
        `INSERT INTO orb_messages
           (orb_id, message_id, content, status, auto_start, created_at, updated_at)
         VALUES ($1,$2,$3,'queued',$4,$5,$5) RETURNING *`,
        [params.orbId, params.messageId, params.content, autoStart, new Date(params.now)],
      );
      if (inserted.isErr()) return err(inserted.error);
      if (state === "stopped" || state === "failed") {
        const started = await query(
          `UPDATE orbs SET state = 'starting', state_version = state_version + 1,
             state_changed_at = $2, updated_at = $2, last_error = NULL,
             stop_reason = NULL, last_busy_at = $2 WHERE id = $1 RETURNING *`,
          [params.orbId, new Date(params.now)],
        );
        if (started.isErr()) return err(started.error);
        orbRow = started.value.rows[0] ?? orbRow;
      } else {
    ...
    return this.db
      .query(
        `SELECT * FROM orb_messages WHERE orb_id = $1
           AND status IN ('queued', 'delivering') ORDER BY ordinal LIMIT 1`,
        [orbId],
      )
      .map((result) => (result.rows[0] === undefined ? null : mapMessageRow(result.rows[0])));
  }
  ...
  noteOrbMessageDelivery(
    ...
    return this.db
      .query(
        `UPDATE orb_messages SET status = 'delivering', delivery = $3,
           operation_id = $4, auto_start = false, updated_at = $5
